...
export default function IDE() {
  ...
  function handleAiSend() {
    ...
    aiChat.mutate({
      id: projectId,
      data: { message: msg, context: selectedFile?.content ?? null, currentFile: selectedFile?.name ?? null },
    }, {
      onSuccess: (data) => {
        setAiMessages(prev => [...prev, { role: "assistant", content: data.reply, codeBlocks: data.codeBlocks }]);
      },
      onError: () => {
        setAiMessages(prev => [...prev, { role: "assistant", content: "Sorry, encountered an error. Please try again." }]);
      },
    });
  }
    ...
    <>
      {selectedFileId && selectedFile ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center gap-3 h-full">
          ...
          <Button size="sm" variant="outline" className="gap-1.5" onClick={() => { setShowNewFile(true); setMobileTab("files"); }}>
  ...
  const aiPanel = (
    <div className="flex flex-col h-full overflow-hidden bg-card">
      ...
      <div className="flex-1 overflow-auto ide-scroll p-3 space-y-3">
                ...
                <div className="mt-2 space-y-1">
                  {msg.codeBlocks.map((cb, ci) => (
                    <div key={ci} className="rounded bg-background border border-border overflow-hidden">
                      <div className="px-2 py-1 text-xs text-muted-foreground border-b border-border font-mono">{cb.filename ?? cb.language}</div>
                      <pre className="p-2 text-xs font-mono overflow-auto max-h-48 ide-scroll text-foreground"><code>{cb.code}</code></pre>
                    </div>
                  ))}
                </div>
              )}
        ...
        {aiChat.isPending && (
          <div className="text-xs"><div className="inline-block bg-muted rounded-lg px-3 py-2"><Loader2 size={12} className="animate-spin" /></div></div>
        )}
        <div ref={chatEndRef} />
      </div>
      <div className="p-2 border-t border-border shrink-0">
        <div className="flex gap-1.5">
          ...
          />
          <Button size="icon" className="h-9 w-9 shrink-0" onClick={handleAiSend} disabled={aiChat.isPending || !aiInput.trim()}>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2">
              ...
              <Input className="font-mono" placeholder="index.ts" value={newFileName} onChange={(e) => setNewFileName(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") handleCreateFile(); }} autoFocus />
            </div>
          </div>
          <DialogFooter>
